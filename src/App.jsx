  ...
  return (
    <div style={{ fontFamily: FONT }} className="min-h-screen bg-slate-50 text-slate-800">
      ...
      <footer className="mx-auto max-w-6xl px-4 pb-8 text-xs leading-relaxed text-slate-400">
        エラーコードは支払基金「受付・事務点検ASPに係るチェック一覧」の医科版・DPC版からの抜粋で、各コードに対象（医科／DPC／共通）を付与しています。コード番号は点数表（医科／DPC）で一部異なるため、本試作では共通概念として集約しており、本番では各公式一覧の正式コードでマスタ管理します（医科は約570件、DPCは約930件、いずれも診療報酬改定ごとに更新）。増減点連絡書の分析は初期表示がサンプルデータで、審査機関から配信される原本形式CSV（レコード種別付き・Shift_JIS対応）や、非識別化した簡易CSV（列：診療年月／診療科／事由／区分／項目名／増減点数）を読み込むと自院の傾向を表示します。アップロードしたデータはブラウザ内でのみ処理され、外部へ送信されません。査定可否の最終判断は担当者が行う前提の、気づきと管理を支援するツールです。
      </footer>
    </div>
  );
}
